...
def interleaveQueue(q):
    ...
    for i in range(size//2):
        s.push(q.dequeue()) #push half to stack
    for skibidi in range(size):
        if skibidi % 2 != 0: #odd the take from stack
            q.enqueue(s.pop())
        else:
            q.enqueue(q.dequeue()) #take from the front to the back
    for rizz in range(size-1): #flips it
        s.push(q.dequeue())
    for huzz in range(size-1):
        q.enqueue(s.pop())
